...
AgroNet is a web platform designed for wholesale crop trading, connecting sellers and consumers seamlessly. Although still in development, AgroNet offers a robust set of features for both sellers and consumers in the agricultural marketplace.
...
1. **Dual Interfaces**: AgroNet provides separate interfaces for consumers and sellers, accessible through the navbar with options for SignUp and SignIn including email verification for the created account.
...
To run AgroNet locally, ensure you have NodeJS and MongoDB installed. Follow these steps:
...
2. **Frontend Setup**:
    - Navigate to the AgroNet `client` folder.
    ...
    - Add the following environment variables to the client `.env` file:
        ```plaintext
        VITE_AgroNet_API = "https://AgroNet-backend.vercel.app/" 
...
3. **Backend Setup**:
    - Navigate to the AgroNet `server` folder.
...
By following these steps, you'll have the AgroNet application running locally on your machine. Adjust configurations as needed for your development environment.

## Contribution

AgroNet welcomes contributions from the community. Feel free to open issues or submit pull requests to help improve the platform.
